...
private:
    ...
    const double tire_B = 1.1559;
    const double tire_C = 1.1924;
    const double tire_D = 0.9956;
    const double tire_E = -0.8505;
    const double tire_Sh = -0.0540;
    const double tire_Sv = 0.1444;
    
    // ** State Propagation (time parameters) ** //
    const double deltaT = 0.001; // dT increment for state propagation (s)
    const double timeStep = 0.1; // length of time for each prediction step in the horizon (s)
